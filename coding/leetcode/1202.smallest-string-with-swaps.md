...
# Complexity
**Solution 1:**
**Time:** O(P*α(N) + N + G*log(G)) - P pairs for union operations with inverse Ackermann, N to group indices, G*log(G) for sorting each group where G is group size
**Space:** O(N) - Union-Find structure, groups dictionary, and result array all use linear space

**Solution 2:**
**Time:** O(P + N + G*log(G)) - P to build graph, N for DFS traversal, G*log(G) for sorting each connected component
**Space:** O(P + N) - Graph adjacency list stores P edges, plus visited array and recursion stack for DFS
