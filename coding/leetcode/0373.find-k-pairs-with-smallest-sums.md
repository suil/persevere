...
# Complexity
**Solution 1:**
**Time:** O(k log k) - We perform k iterations, each involving heap operations (add/poll) that take O(log k) time, and we initially add at most k elements to the heap
**Space:** O(k) - The heap stores at most k elements at any time, and the result array stores k pairs

**Solution 2:**
**Time:** O(n log k) - We iterate through n elements, performing heap add operation O(log k) for each element and poll operation O(log k) when heap exceeds size k
**Space:** O(k) - The heap maintains at most k elements
