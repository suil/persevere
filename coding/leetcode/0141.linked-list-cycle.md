# Linked List Cycle
[141. Linked List Cycle](https://leetcode.com/problems/linked-list-cycle/)

Given head, the head of a linked list, determine if the linked list has a cycle in it.

There is a cycle in a linked list if there is some node in the list that can be reached again by continuously following the next pointer. Internally, pos is used to denote the index of the node that tail's next pointer is connected to. Note that pos is not passed as a parameter.

Return true if there is a cycle in the linked list. Otherwise, return false.

Example 1:

Input: head = [3,2,0,-4], pos = 1
Output: true
Explanation: There is a cycle in the linked list, where the tail connects to the 1st node (0-indexed).

Example 2:
Input: head = [1,2], pos = 0
Output: true
Explanation: There is a cycle in the linked list, where the tail connects to the 0th node.

Example 3:
Input: head = [1], pos = -1
Output: false
Explanation: There is no cycle in the linked list.

# Key insights
* Floyd's cycle detection algorithm using fast and slow pointers
* Fast pointer moves 2 steps, slow pointer moves 1 step per iteration
* If cycle exists, fast pointer will eventually meet slow pointer
* O(1) space complexity avoids using hash set to track visited nodes

# Code
```javascript
var hasCycle = function(head) {
    if (!head) return false;
    
    let slowPointer = head;
    let fastPointer = head.next;
    
    while (slowPointer !== fastPointer) {
        if (!fastPointer || !fastPointer.next) {
            return false;
        }
        slowPointer = slowPointer.next;
        fastPointer = fastPointer.next.next;
    }
    
    return true;
};
```

# Complexity
**Time:** O(n) - In the worst case, the slow pointer traverses at most n nodes before either meeting the fast pointer or the fast pointer reaches the end
**Space:** O(1) - Only uses two pointer variables regardless of input size
