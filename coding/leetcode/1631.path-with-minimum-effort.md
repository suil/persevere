...
# Key insights
- Effort = maximum absolute height difference along the path
- DP approach: repeatedly relax distances by considering all neighbors
- dp[i][j] = minimum effort to reach cell (i,j)
- Alternative: binary search on effort + DFS to check reachability
...
# Complexity
**Time:** O(rows * cols * max(rows, cols)) - DP with multiple passes
