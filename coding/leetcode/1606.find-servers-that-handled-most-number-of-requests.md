...
# Key insights
- Use min-heap to track busy servers (sorted by when they become free)
- Use sorted array for available servers with binary search
- For request i, prefer server (i % k), wrap around if needed
- Free up servers whose tasks completed before current arrival time
...
# Complexity
**Time:** O(n log k) - n requests, each with log k heap operations and binary search
**Space:** O(k) - for tracking k servers
