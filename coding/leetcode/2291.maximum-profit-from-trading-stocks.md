...
# Key insights
* This is a 0/1 knapsack problem — each stock is an item with cost `present[i]` and profit `future[i] - present[i]`
...
# Complexity
**Time:** O(n * budget) - For each stock, iterate through all possible budget values.
**Space:** O(budget) - 1D DP array of size budget + 1.
