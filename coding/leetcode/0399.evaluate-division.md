...
# Complexity
**Solution 1:**
**Time:** O(Q * (V + E)) - Each of the Q queries performs DFS which visits at most V vertices and E edges in worst case
**Space:** O(V + E) - Graph storage requires space for V vertices and E edges, plus O(V) for recursion stack and visited set

**Solution 2:**
**Time:** O((E + Q) * α(V)) - Union-Find operations with path compression have nearly constant amortized time, applied to E equations and Q queries
**Space:** O(V) - Only stores the roots mapping for Union-Find structure
