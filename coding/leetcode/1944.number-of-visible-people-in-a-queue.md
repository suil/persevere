...
* For each person, pop all shorter people from the stack — each popped person is visible to the current person
...
# Complexity
**Time:** O(n) - Each element is pushed and popped from the stack at most once.**Space:** O(n) - The stack can hold up to n elements in the worst case.
