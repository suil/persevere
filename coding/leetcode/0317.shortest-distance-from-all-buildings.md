...
# Key insights
* Run BFS from each building to calculate distances to all reachable empty land
* Track both cumulative distances and reach counts for each empty cell across all BFS runs
* Only consider empty cells reachable by ALL buildings as valid house placement candidates
* Use level-by-level BFS with distance tracking to ensure shortest paths from each building
...
# Complexity
**Time:** O(B * m * n) - where B is the number of buildings, m is rows, and n is columns, since we perform BFS from each building traversing the entire grid in worst case
**Space:** O(m * n) - for the distances, reaches, visited arrays, and BFS queue which can contain up to m*n cells
