...
- Use monotonic stack to find previous smaller and next smaller elements
- Element at index i is minimum for (left choices) * (right choices) subarrays
- Contribution = arr[i] * (i - prevSmaller[i]) * (nextSmaller[i] - i)
