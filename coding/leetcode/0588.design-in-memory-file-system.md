...
# Complexity
**Time:** O(m + n log n) for ls where m is path length and n is number of entries; O(m) for other operations.**Space:** O(total content + total paths) - Storing all file contents and directory structure.
