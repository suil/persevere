...
# Key insights
- Sort cars by position descending (cars closest to target first)
- Calculate time to reach target for each car: (target - position) / speed
- Use a stack: if current car takes longer than the car in front, it forms a new fleet
- Cars that catch up merge into the fleet ahead (same arrival time)
...
**Space:** O(n) - for storing cars and the stack
