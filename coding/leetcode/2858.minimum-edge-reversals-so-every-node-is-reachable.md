...
* Re-root the tree using a second DFS: when moving the root from parent to child, if the edge is forward (parent→child), we need one more reversal; if reverse, one fewer
...
# Complexity
**Time:** O(n) - Two DFS passes, each visiting every node and edge once.**Space:** O(n) - Adjacency list, result array, and recursion stack use linear space.
