...
* Order of operations on the same element matters — try both orderings and take the minimum
...
# Complexity
**Time:** O(n * op1 * op2) - Each unique state (index, op1, op2) is computed once.**Space:** O(n * op1 * op2) - Memoization table storing all computed states, plus recursion stack of depth n.
