...
# Complexity
**Solution 1:**
**Time:** O(n²) - Each unique (current, prevNum) state is computed once, with n possible current values and up to n distinct prevNum values
**Space:** O(n²) - Memoization map stores results for all unique state combinations plus recursion stack depth of O(n)

**Solution 2:**
**Time:** O(n²) - Nested loops where outer loop runs n times and inner loop runs up to i times for each outer iteration
**Space:** O(n) - Only uses a single dp array of size n to store the length of LIS ending at each position
