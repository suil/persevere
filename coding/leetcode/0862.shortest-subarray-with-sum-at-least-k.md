...
- Monotonic deque stores indices of increasing prefix sums
- Pop from front when prefixSum[i] - prefixSum[front] >= k (found valid subarray)
- Pop from back if current prefix sum is smaller (smaller start index would be better)
